...
var driver = {
	"firefox": "",
	"chrome": `-Dwebdriver.chrome.driver=${config.driverPath.chrome}`,
	"phantomjs": `-Dwebdriver.phantomjs.driver=${config.driverPath.phantomjs}`
};
...
function createServer(args, callback) {
	...
	function processData(data) {
		...
		if (data.search("Address already in use") !== -1 ||
			data.search("Selenium is already running on port") !== -1) {
